...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: ./VideoShared <input_video> <output_video>" << std::endl;
        return 1;
    }
    ...
    std::cout << "Algorithm: OPTIMIZED SHARED MEMORY" << std::endl;
    ...
    int blockSize = 128; // Use the optimal block size found in experiments
    ...
    while (cap.read(frame)) {
        ...
        float ms = run_gpu_shared_frame(pixels, width, height, labels, centroids, blockSize);
    ...
    std::cout << "\n==============================================" << std::endl;
    std::cout << "OPTIMIZED GPU SUMMARY" << std::endl;
